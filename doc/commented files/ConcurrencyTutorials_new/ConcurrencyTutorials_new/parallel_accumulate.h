...
void parallel_accumulate(iterator start, iterator end, T &ref)
{
	...
	//this is due to the fact that some operating system may return 0 to indicate that this has not implemented
	...
	results[allowed_threads - 1] =std::accumulate(start, end, results[allowed_threads - 1]);
